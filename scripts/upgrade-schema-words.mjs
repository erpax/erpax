...
const walk = (d) => {
  for (const e of readdirSync(d)) {
    ...
    if (!/a schema\.org (component|vocabulary) word/.test(s)) continue // schema.org word-atoms only
    if (/\*\*Law/.test(s)) continue // already sealed
    const name = basename(dirname(p))
    // 1. "Use when" description (replace the legacy quoted line; no colon-space → js-yaml safe)
    const desc =
