...
import {
  ...
  RoleScopedDataSeed,
  Level2SeedSuite,
} from '@/plugins/accounting/seeds/level-2';
import { MinimalHostSeed, MinimalGLAccountsSeed } from '@/plugins/accounting/seeds/level-1';
...
class MockPayload implements Partial<Payload> {
  ...
  config: any = {
    collections: [
      ...
      },
      {
        ...
        fields: [
          { name: 'tenant', type: 'relationship', required: true, relationshipTo: 'hosts' },
  ...
  async create({ collection, data }: { collection: string; data: any }): Promise<any> {
    ...
    const id = `${collection}-${(this.idCounter.get(collection) || 0) + 1}`;
    this.idCounter.set(collection, (this.idCounter.get(collection) || 0) + 1);

    const doc = { ...data, id };
    this.documents.get(collection)!.push(doc);
    return doc;
  }
  ...
  async delete({ collection, id }: { collection: string; id: string }): Promise<void> {
    const docs = this.documents.get(collection) || [];
