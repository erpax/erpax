/**
 * Intercompany Consolidation workflow walk-through — auto-scaffolded by
 * `erpax.consistency.applyAll` (Slice LLLLLLLL 2026-05-11) to close
 * the marketing/multimedia seed gap for BUSINESS_CHAINS.INTERCOMPANY_CONSOLIDATION.
 *
...
const WORKFLOW = 'intercompany-consolidation'
const BASE = 'http://localhost:3000'

test.describe('ERP workflow: Intercompany Consolidation', () => {
  test.describe.configure({ timeout: 180_000 })

  test('walk-through: Intercompany transaction with consolidation elimination', async ({ page }, testInfo) => {
    await login({ page, user: testUser })
    await captureWorkflowStep(page, testInfo, WORKFLOW, '00-dashboard',
      'Admin dashboard after login')
    // TODO(INTERCOMPANY_CONSOLIDATION): expand step list per the chain's BUSINESS_CHAINS
    ...
    await safeCaptureRoute({ page, testInfo, workflow: WORKFLOW,
      stepId: '01-overview',
      label: 'Intercompany transaction with consolidation elimination overview',
      route: `${BASE}/admin`,
    })
  })
})
