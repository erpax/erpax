/**
 * H2r Hire To Retire workflow walk-through — auto-scaffolded by
 * `erpax.consistency.applyAll` (Slice LLLLLLLL 2026-05-11) to close
 * the marketing/multimedia seed gap for BUSINESS_CHAINS.H2R_HIRE_TO_RETIRE.
 *
...
const WORKFLOW = 'h2r-hire-to-retire'
const BASE = 'http://localhost:3000'

test.describe('ERP workflow: H2r Hire To Retire', () => {
  test.describe.configure({ timeout: 180_000 })

  test('walk-through: Hire-to-Retire', async ({ page }, testInfo) => {
    await login({ page, user: testUser })
    await captureWorkflowStep(page, testInfo, WORKFLOW, '00-dashboard',
      'Admin dashboard after login')
    // TODO(H2R_HIRE_TO_RETIRE): expand step list per the chain's BUSINESS_CHAINS
