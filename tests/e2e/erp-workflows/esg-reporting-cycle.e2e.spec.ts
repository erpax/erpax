/**
 * Esg Reporting Cycle workflow walk-through — auto-scaffolded by
 * `erpax.consistency.applyAll` (Slice LLLLLLLL 2026-05-11) to close
 * the marketing/multimedia seed gap for BUSINESS_CHAINS.ESG_REPORTING_CYCLE.
 *
...
const WORKFLOW = 'esg-reporting-cycle'
const BASE = 'http://localhost:3000'

test.describe('ERP workflow: Esg Reporting Cycle', () => {
  test.describe.configure({ timeout: 180_000 })

  test('walk-through: ESG reporting cycle (CSRD)', async ({ page }, testInfo) => {
    await login({ page, user: testUser })
    await captureWorkflowStep(page, testInfo, WORKFLOW, '00-dashboard',
      'Admin dashboard after login')
    // TODO(ESG_REPORTING_CYCLE): expand step list per the chain's BUSINESS_CHAINS
    ...
    await safeCaptureRoute({ page, testInfo, workflow: WORKFLOW,
      stepId: '01-overview',
      label: 'ESG reporting cycle (CSRD) overview',
      route: `${BASE}/admin`,
    })
  })
})
