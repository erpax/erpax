/**
 * O2c Goods workflow walk-through — auto-scaffolded by
 * `erpax.consistency.applyAll` (Slice LLLLLLLL 2026-05-11) to close
 * the marketing/multimedia seed gap for BUSINESS_CHAINS.O2C_GOODS.
 *
...
const WORKFLOW = 'o2c-goods'
const BASE = 'http://localhost:3000'

test.describe('ERP workflow: O2c Goods', () => {
  test.describe.configure({ timeout: 180_000 })

  test('walk-through: Order-to-Cash (physical goods)', async ({ page }, testInfo) => {
    await login({ page, user: testUser })
    await captureWorkflowStep(page, testInfo, WORKFLOW, '00-dashboard',
      'Admin dashboard after login')
    // TODO(O2C_GOODS): expand step list per the chain's BUSINESS_CHAINS
    ...
    await safeCaptureRoute({ page, testInfo, workflow: WORKFLOW,
      stepId: '01-overview',
      label: 'Order-to-Cash (physical goods) overview',
      route: `${BASE}/admin`,
    })
  })
})
