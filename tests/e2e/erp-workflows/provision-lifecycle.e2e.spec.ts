/**
 * Provision Lifecycle workflow walk-through — auto-scaffolded by
 * `erpax.consistency.applyAll` (Slice LLLLLLLL 2026-05-11) to close
 * the marketing/multimedia seed gap for BUSINESS_CHAINS.PROVISION_LIFECYCLE.
 *
...
const WORKFLOW = 'provision-lifecycle'
const BASE = 'http://localhost:3000'

test.describe('ERP workflow: Provision Lifecycle', () => {
  test.describe.configure({ timeout: 180_000 })

  test('walk-through: Provision lifecycle (IAS-37)', async ({ page }, testInfo) => {
    await login({ page, user: testUser })
    await captureWorkflowStep(page, testInfo, WORKFLOW, '00-dashboard',
      'Admin dashboard after login')
    // TODO(PROVISION_LIFECYCLE): expand step list per the chain's BUSINESS_CHAINS
    ...
    await safeCaptureRoute({ page, testInfo, workflow: WORKFLOW,
      stepId: '01-overview',
      label: 'Provision lifecycle (IAS-37) overview',
      route: `${BASE}/admin`,
    })
  })
})
