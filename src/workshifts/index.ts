...
if (!ring.ok) throw new Error(`workshifts: horo disharmony — ${ring.errors.join('; ')}`)
...
export const requireWageToClose: CollectionBeforeChangeHook = ({ data }) => {
  ...
  if (d.status === 'closed' && (d.wage === undefined || d.wage === null)) {
    throw new Error(
      'workshifts: a labour-day closes only once reconciled into a wage — the authority must settle before orders inherit it.',
    )
  }
  return data
}
