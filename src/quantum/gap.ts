 ...
 *
 * Gate axis name is `linear-gap`; file path is single-word `gap.ts` (folder law).
 */
...
import { interact64, architectureMask } from '@/quantum/word'
...
const uuidFold64 = (uuid: string): bigint => {
  const h = uuid.replace(/[^0-9a-fA-F]/g, '').slice(0, 16)
  return h.length > 0 ? BigInt(`0x${h}`) : 0n
}
const wordFold = (atomPath: string): bigint =>
  uuidFold64(wordTokenUuid(atomPath.split('/').pop() ?? atomPath)) & architectureMask()
const digitFold = (atomPath: string): bigint => {
...
export function entanglementScore(atomPath: string, other?: string): bigint {
  const base = interact64(wordFold(atomPath), digitFold(atomPath))
  return other ? interact64(base, interact64(wordFold(other), digitFold(other))) : base
}
...
const dedupe = (gaps: readonly LinearGap[]): LinearGap[] => {
...
export async function sealLinearGaps(
  ...
  for (const gap of beforeScan.gaps.slice(0, max)) {
    ...
    switch (gap.sealHint.action) {
      case 'stub-index': {
        const dir = join(cwd, SRC, gap.atomPath)
