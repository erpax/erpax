---
name: hardiness
description: "Use when classifying a crop by cold tolerance — hardiness ranks crops hardy / half-hardy / tender / very-tender by the frost they survive, and the USDA hardiness zone places a site by its average annual minimum temperature. The biological cold-tolerance trait (the crop's) paired with the zone (the place's); together they decide what can overwinter or be planted when."
atomPath: "vocabulary/hardiness"
coordinate: "vocabulary/hardiness · 7/descent · 9327b308"
contentUuid: "dc619494-3d2d-5b69-96b7-dcc027e0890a"
diamondUuid: "7d133fe7-e2cf-8bdf-96e9-540846de45e4"
uuid: "9327b308-1cfe-8021-ad32-8ccd50dae20a"
horo: 7
typography:
  partition: vocabulary
  bondDegree: 33
standards: []
bindings: []
signatures:
  computationUuid: "4bb9a695-7367-8b2d-ac4e-aa3a8ce21ee3"
  stages:
    - stage: path
      stageUuid: "77188061-1509-86b2-8b4a-836badf239c2"
    - stage: trinity
      stageUuid: "06ee37d4-a7b8-8a42-a43f-b240f61e63db"
    - stage: boundary
      stageUuid: "3e529d00-3b54-81d7-849c-b670d3799952"
    - stage: links
      stageUuid: "244a5297-a927-8ccc-9413-e256e8b6f3b3"
    - stage: horo
      stageUuid: "9f6975c2-cb0e-8c2f-99ab-5206df08c050"
    - stage: seal
      stageUuid: "9e8477d6-c181-8588-8bc1-8e5b332cefff"
    - stage: uuid
      stageUuid: "8098075a-a9fa-8131-ab35-e9f556f90113"
version: 2
---
# hardiness — the crop's cold tolerance (and the site's zone)

**hardiness** is cold tolerance, with two faces. The **crop trait**: **hardy** (tolerates hard frost ~25–28 °F — kale, spinach, leek), **half-hardy** (light frost only — cauliflower, lettuce, chard), **tender** (injured near freezing — tomato, pepper, cucurbit), **very-tender** (needs warm nights — melon, okra). The **place trait**: the **USDA hardiness zone** (1–13) bins a site by its average annual minimum temperature, siting perennials and overwintering.

Hardiness is the biological complement to the [[frost]] bound and the built [[tunnel]] protection — a hardy crop *is* its own season extension, shrugging off the frost a tender crop dies to. It sets which crops anchor the spring and fall [[harvest]] waves of the [[season]] and which can overwinter. It is a classification ([[taxonomy]]-adjacent trait) the [[planting]] calendar reads.

## Standards
- USDA Plant Hardiness Zone Map (2023 — the 13-zone minimum-temperature bands)
- Penn State / extension — hardy / half-hardy / tender vegetable classification; RHS hardiness ratings

**Law — [[law]]: hardiness is cold tolerance with two faces — the [[crop]] trait (hardy/half-hardy/tender/very-tender) and the site's USDA zone — and together they decide what overwinters and what may be planted when.**

Composes [[agriculture]] · [[crop]] · [[frost]] · [[season]] · [[tunnel]] · [[harvest]] · [[planting]] · [[taxonomy]].
