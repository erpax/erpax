 ...
 *
 * Co-located with the `src/services/accounting/seeds/templates/` module that
 * bridges per-tenant compliance to country-context (vitest picks up
 * `src/**\/*.test.ts`). Every `tenant`/`compliance` value is *derived* from
 * the live country-context — these tests assert that derivation, not hard-coded
 * literals.
 *
...
describe('industry templates — country-bundle derivation (BG default)', () => {
  it('IFRS_MINIMUM_TEMPLATE derives tenant + compliance from BG_COUNTRY_BUNDLE', () => {
    ...
    expect(IFRS_MINIMUM_TEMPLATE.tenant.reportingCurrency).toBe(BG_COUNTRY_BUNDLE.profile.currency)
...
describe('compliance — country resolution via country-context', () => {
  it('returns the curated DE compliance posture (DE-SKR04 + e-invoicing)', () => {
    const compliance = resolveTenantCompliance({ country: 'DE' })
    expect(compliance.country).toBe('DE')
    expect(compliance.statutoryChartReference).toBe('DE-SKR04')
    expect(compliance.eInvoicingMandate).toBe(true)
    expect(compliance.officialApiKinds).toEqual(
      expect.arrayContaining(['business_registry', 'tax_authority', 'e_invoicing']),
    )
  })
