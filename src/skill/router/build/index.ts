...
function writeSkillIndexFile(cwd: string, index: readonly SkillNode[], note: string): { count: number; bytes: number; out: string } {
...
/**
 * Empty expert pool — satisfies the static import for Payload migrate / OpenNext
 * build without baking 80MB into the Worker (hard 3MB script limit).
 */
export function buildSkillIndexStub(cwd: string = process.cwd()): { count: number; bytes: number; out: string } {
