<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# properties

> atom `properties/investment/properties` · horo `8` `crest` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[trinity]]/code `1` |  |
| [[asset]]/[[trinity]]/proof `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `12` |  |
| Σ `21` | Σ `0` |

> [[balance]] `21` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `properties` · sub-root `76e25ce1-4d4b-8d90-971e-ac1477bf571c`
- bond degree `25` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `bf1a0fd6-5666-8be1-8753-c3c46cb4a444`
- bonds in `6` · out `6`
- trinity form·code·proof `1`·`1`·`1`
- links `3` / `3`
- folded `1` · escapes `0`

## typography graph

- partition `properties`
- partition root `76e25ce1-4d4b-8d90-971e-ac1477bf571c`
- bond degree `25`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `09c1a5c9-3ed8-8fce-a57e-1a2d69e8c565` · framed by typography partition `properties` bonds `25` · `pnpm readme` · `pnpm readme:check`</sub>
