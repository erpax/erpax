 ...
 *
 * Runtime law gates (dispatch · effect · MCP) live in this cross — not a stray sibling.
 *
 * @standard NIST SP 800-162 ABAC · OWASP-LLM-Top-10:2025 LLM01
 */
import {
  cascadeDepthVerdict,
  groundToolCall,
...
export function recordPathVisits(
  ...
  for (const raw of paths) {
    ...
    if (ledger) {
      ...
      ledger.push(recordOnPath(p, { kind: 'dispatch.visit', eventId }, at, prev, seq))
    }
  }
}
...
const rulesAxesForEffectGate = (
  law: AgentLawState,
): readonly { axis: string; violations: number; baseline: number }[] => {
    ...
    case 'update': {
      ...
      if (unsealed.length > 0) {
        ...
        if (!crossGate.allowed) {
          ...
          const atomPath = [...(law.pathsVisited ?? [])][0] ?? unsealed[0]?.atomPath ?? 'rules'
          crossViolationFromStrictApply({
            atomPath,
            gate: 'effect',
