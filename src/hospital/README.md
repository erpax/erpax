<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# hospital

> atom `hospital` · horo `8` `crest` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `4` |  |
| Σ `11` | Σ `0` |

> [[balance]] `11` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `hospital` · sub-root `9faf3f4e-733f-82e8-b101-3f602b68b767`
- bond degree `7` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `9faf3f4e-733f-82e8-b101-3f602b68b767`
- bonds in `2` · out `2`
- trinity form·code·proof `1`·`0`·`0`
- links `2` / `2`
- folded `1` · escapes `0`

## typography graph

- partition `hospital`
- partition root `9faf3f4e-733f-82e8-b101-3f602b68b767`
- bond degree `7`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `57bf0e1d-a42e-875f-83d0-48c5fb391b60` · framed by typography partition `hospital` bonds `7` · `pnpm readme` · `pnpm readme:check`</sub>
