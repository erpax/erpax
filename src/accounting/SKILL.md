...
atomPath: accounting
coordinate: "accounting · 7/descent · babd2270"
contentUuid: "d8bd0827-ca01-5419-b949-e4b06041b00f"
diamondUuid: "a4f64c6d-1cc0-8db2-8916-940e35fa38df"
...
bindings: []
signatures:
  computationUuid: "b55e1818-3cb4-899c-9cf0-28bd3ef11d2a"
  stages:
    - stage: path
    ...
    - stage: horo
      stageUuid: "6196a506-18ec-8fc7-a969-020dd6b97584"
    - stage: seal
      stageUuid: "c3103fdd-ed65-8eb6-89d0-e6cb44152940"
    - stage: uuid
      stageUuid: "f1792c0d-01ec-898c-b147-4dc4b7942cb2"
version: 2
---
...
**Law — [[law]]: every posting is a debit⊕credit⊕balance trinity and no entry writes unless Σdebit = Σcredit, so the whole content-addressed ledger nets to zero in all directions.**

**Law — [[law]]: path is the account code — the fs tree fold IS the chart of accounts; no separate GL catalogue.** Every atom folder (`law/folder`, `readings`, `quantum/emr`) posts to `accountCode = accountCodeOf(atomPath)` (full path, homonym-safe — `agents/accounting` ≠ `accounting`). When erpax accounts itself, the currency is **eb** (entropy-bit: tamper-cost log₂ mass at the horo imperial-ratio floor); gap debits Dr the folder path / Cr `entropy`; seal credits Dr `seal` / Cr the folder path — `postEntry` · `erpaxSelfAccount` · `accountCorpusEntropy` materialize `journal-entries` documents; `accountCoordinateOf` pairs each code with `coordinateAddress` for audit binds.
