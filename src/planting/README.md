<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# planting

> atom `planting` · horo `5` `round` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `4` |  |
| Σ `11` | Σ `0` |

> [[balance]] `11` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `planting` · sub-root `12ae433c-305d-81e6-a2d3-ed3c085a7ba7`
- bond degree `39` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `12ae433c-305d-81e6-a2d3-ed3c085a7ba7`
- bonds in `2` · out `2`
- trinity form·code·proof `1`·`0`·`0`
- links `33` / `33`
- folded `1` · escapes `0`

## typography graph

- partition `planting`
- partition root `12ae433c-305d-81e6-a2d3-ed3c085a7ba7`
- bond degree `39`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `14162aa6-c727-88ba-9434-3ac05fbe9e8b` · framed by typography partition `planting` bonds `39` · `pnpm readme` · `pnpm readme:check`</sub>
