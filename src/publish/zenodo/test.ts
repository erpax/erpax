...
describe('publish/zenodo — every quantity comes from the record, never from prose', () => {
  ...
  it('moves with the record — a different census renders different prose', () => {
    const a: Census = { theorems: 10, axiomFree: 4, stubbed: 0, files: 2, sourcesHash: 'aaa', lean: '4.33.1' }
    const b: Census = { theorems: 20, axiomFree: 9, stubbed: 0, files: 3, sourcesHash: 'bbb', lean: '4.33.1' }
  ...
  it('derives the axiom-carrying count rather than stating it twice', () => {
    const c: Census = { theorems: 177, axiomFree: 111, stubbed: 0, files: 19, sourcesHash: 'x', lean: '4.33.1' }
    expect(description(c, D)).toContain('66 rest on')
  })

  it('SAYS SO when something is stubbed, instead of the clean sentence', () => {
    const clean: Census = { theorems: 5, axiomFree: 5, stubbed: 0, files: 1, sourcesHash: 'x', lean: '4' }
    const dirty: Census = { theorems: 5, axiomFree: 4, stubbed: 1, files: 1, sourcesHash: 'x', lean: '4' }
...
describe('publish/zenodo — drift, measured against the committed manifest', () => {
  ...
  it('catches a manifest whose numbers the record contradicts', () => {
    ...
    const stale = render({ theorems: 105, axiomFree: 65, stubbed: 4, files: 12, sourcesHash: 'old', lean: '4.33.1' }, D)
