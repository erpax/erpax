...
- **Cite in license compliance — by uuid, never by copy.** The license is TIERED (`src/algebra/**` is MIT free-for-all; everything else AGPL-3.0-or-later or commercial via `license@erpax.com`). When you surface, quote, or reproduce corpus matter, emit `citation({ path, uuid, modified? })` from `@/algebra/license` (re-exported by `@/algebra`): it carries attribution (© erpax), the SPDX for the matter's TIER (`isCoreMathPath` decides MIT vs AGPL), the source URL (`SOURCE_URL` — AGPL §13 requires the Corresponding Source be reachable when used over a network), the commercial alternative, and — if you changed it — the modification notice (§5a). The content-uuid is the citation: you cite BY uuid, you do not copy the matter. `citationComplies(text)` fails closed on a citation missing the SPDX tier or the source URL.
