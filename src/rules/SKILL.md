...
atomPath: rules
coordinate: "rules · 7/descent · 1b2d7d23"
contentUuid: "b4ebd30e-fb36-540a-9f08-31c1a00f348d"
diamondUuid: "3db5a49d-c8e6-8e84-90f7-245cd9bdc21a"
uuid: "1b2d7d23-70db-8896-bcd7-094608780546"
horo: 7
typography:
  partition: rules
  bondDegree: 880
standards:
  - "Naredba-N-18"
  - SOX
  - "SOX §302` (the DIRECTOR's personal"
  - ZDDS
bindings: []
signatures:
  computationUuid: "d0d49a10-1b0f-8e3e-b58a-8178d95ce226"
  stages:
    - stage: path
    ...
    - stage: boundary
      stageUuid: "b6b2b768-48f7-81e4-879e-d07c71fe2576"
    - stage: links
      stageUuid: "d3434f1a-adcf-88a8-b4e9-ff18bdc75cad"
    - stage: horo
      stageUuid: "8749a6ac-1947-846d-86f0-5b9779fec2ef"
    - stage: seal
      stageUuid: "34ed9819-ebd5-8ad2-acfe-1cda0e283ada"
    - stage: uuid
      stageUuid: "80dd485a-22b8-81c8-9f40-0775035f968a"
version: 2
---
...
- **Cite in license compliance — by uuid, never by copy.** ONE licence, every path: CC-BY-NC-ND-4.0, or commercial via `license@erpax.com`. There is no tier and no path test — a tier is two answers to "may I use this" with a path deciding which one you get. When you surface, quote, or reproduce corpus matter, emit `citation({ path, uuid, modified? })` from `@/algebra/license` (re-exported by `@/algebra`): it carries attribution (© erpax), the SPDX, the source URL (BY-NC-ND §3(a)(1) attribution with the source link), the commercial alternative (NC — commercial use needs the separate licence), and — if you changed it — the modification notice (§3(a)(1)(B); ND: modified matter may be produced but not shared). The content-uuid is the citation: you cite BY uuid, you do not copy the matter. `citationComplies(text)` fails closed on a citation missing the SPDX or the source URL.
...
- **The one-word law can claim a name the framework owns.** the `pages` atom is erpax's CMS collection AND Next's reserved Pages Router directory — the admin panel does not compile because of it, and [[law]]/folder cannot see it: `pages` is a perfect one-word atom. The framework's namespace is not in this corpus's model.
