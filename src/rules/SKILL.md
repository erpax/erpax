...
- **Cite in license compliance — by uuid, never by copy.** The license is TIERED (`src/algebra/**` is MIT free-for-all; everything else CC-BY-NC-ND-4.0 or commercial via `license@erpax.com`). When you surface, quote, or reproduce corpus matter, emit `citation({ path, uuid, modified? })` from `@/algebra/license` (re-exported by `@/algebra`): it carries attribution (© erpax), the SPDX for the matter's TIER (`isCoreMathPath` decides MIT vs BY-NC-ND), the source URL (BY-NC-ND §3(a)(1) attribution with the source link), the commercial alternative (NC — commercial use needs the separate licence), and — if you changed it — the modification notice (§3(a)(1)(B); ND: modified matter may be produced but not shared). The content-uuid is the citation: you cite BY uuid, you do not copy the matter. `citationComplies(text)` fails closed on a citation missing the SPDX tier or the source URL.
