...
- **Use packages canonically — their API, never their binary or a re-implementation.** Check `node_modules` before building: the package has already paid the derivation. Payload IS quantum by architecture and its features ARE these laws — **blocks** = superposition (form-builder: ONE `forms` collection, 13 block types — the canonical "many types, one store"), **nested-docs** = the self-referential tree, **multi-tenant** = scope, **import-export** = the port, **plugin-mcp** = the agent gateway; a collection is *"records that share a common schema"* — a shared **schema**, never a shared word. Extend through the plugin's own override API (`ordersCollectionOverride`). Verified violations: `execFileSync('psql')` instead of the `pg` client (non-hermetic, reparses CLI text); `nestedDocsPlugin` installed and **never called while 9 collections hand-rolled a `parent` tree** (now wired computed in `payload.config.ts` — it *enhances* an existing `parent` with the acyclic `filterOptions` nothing enforced before, and its `breadcrumbs` IS etrima's dropped `ancestry` path); [[variant]]'s `expandVariants` duplicating plugin-ecommerce's `variantTypes ⊗ variantOptions → variants`, which erpax already enables.
- **Research the real app AND data — never port blindly.** The Rails source (`~/github/ceccec/etrima`) is the authority on *intent*; the live DB is the authority on *use*; a summary is neither. **`rows > 0` ≠ used**: `count(col)` counts non-NULL and **0 is non-NULL** — `packing_lists` has 727 rows whose weights are all `min=max=sum=0`, so its mass-balance law held **727/727 vacuously**; a green check on empty data is indistinguishable from a green check on real data (check `min`/`max`/`sum`). `work/shifts` claimed efficiency "falls back to 100" while Rails says `||= employee.work_efficiency || 100` — `||=` *preserves*, **0 is truthy in Ruby**, and 69% of 32 039 real fallback rows recorded **0**: the port would have called ~30 000 no-production shifts "100% efficient". Prefer a fold you **cancel** over one you invent, and record dead columns as honest boundaries ([[port]]).
