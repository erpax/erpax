<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# mail

> atom `mail` · horo `4` `weave` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `14` |  |
| Σ `21` | Σ `0` |

> [[balance]] `21` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `mail` · sub-root `b94d4ecd-6ad3-8c2e-9b7d-1dc568d607d9`
- bond degree `22` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `b94d4ecd-6ad3-8c2e-9b7d-1dc568d607d9`
- bonds in `7` · out `7`
- trinity form·code·proof `1`·`0`·`0`
- links `6` / `6`
- folded `1` · escapes `0`

## typography graph

- partition `mail`
- partition root `b94d4ecd-6ad3-8c2e-9b7d-1dc568d607d9`
- bond degree `22`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `89233b9e-fd83-8ed6-8e01-98a6b4b95b62` · framed by typography partition `mail` bonds `22` · `pnpm readme` · `pnpm readme:check`</sub>
