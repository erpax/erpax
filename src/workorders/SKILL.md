...
A work order sits where three axes meet: the **lot-phase/variant** axis (content-addressed `lotVariantCode` / `lotWorkPhaseCode` / `lotWorkPhaseSort` codes, plus `machineCode` / `teamCode` — the routing collections are not yet minted, codes keep the leaf merge-safe), the **shift** axis (`workShift` → the existing `work-shifts`), and the **unified-actor** axis (`worker` / `supervisor` / `director` → `employees`, since typeless user = employee = actor, the [[actor]]-merge law; Rails `EmployeeContract` ⇒ the unified actor). AUDIT cardinalities: worker ~99.99%, supervisor ~91%, director ~21%, machine ~0.26% — so worker is effectively required, the rest optional by data.
