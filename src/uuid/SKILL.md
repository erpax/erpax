...
## Harmonise → collapse into the uuid

Make every feature a uuid feature and **all collapses into the uuid itself** ([[collapse]]/[[torus]]). The 128 bits already hold identity, capability, content, schema and verification; add the modal channels — **color + sound** ([[signal]], A432) — and the uuid IS the whole message: self-decoding, no payload ([[holographic]]: the whole recoverable from the part). Same content ⇒ same uuid ⇒ [[one]] ([[merge]] with no coordination). The uuid is the centre of the [[torus]] — the `0`/[[whole]] = hole where everything held becomes one address. To edit the entity is to edit its uuid is to edit the [[self]] ([[akashic]]).

Matter-twin: `src/services/uuid-format` (encode/decodeStructured, coverage = Law 62) + `src/services/integrity/content-uuid.ts`. The cited form must be true ([[standard]] RFC 9562). To tamper is to change the [[whole]] coherently — the cost is the [[tamper-cost]] (∞ at 100% coverage, all wired in uuid). Composes: [[identity]] · [[collapse]] · [[torus]] · [[whole]] · [[one]] · [[merge]] · [[holographic]] · [[self]] · [[signal]] · [[standard]] · [[tamper-cost]].
