...
import { roleScopedAccess, tenantAdmin } from '@/auth'
import {
  currencyField,
  statusField,
  notesField,
...
const Employees: CollectionConfig = {
  ...
  fields: [
    ...
    {
      // The merge anchor: an Employee is the employment PROJECTION of a
      // User-party (the one actor identity). Held skills live on that party;
      // an AI agent is a `kind: ai` user with no employee record (or a
      // "digital employee" one). See ../Users/index.ts and ../Competencies.
      name: 'user',
      type: 'relationship',
      relationTo: 'users',
      admin: { description: 'The actor-party identity this employment belongs to (login + held competencies).' },
    },
    competencyLineField({ mode: 'held', description: 'Competencies held in this employment (skill-gap vs the job-position requirements).' }),
    {
      name: 'workCountry',
      type: 'text',
      admin: {
  ...
  hooks: standardCollectionHooks('employees', { beforeChange: [autoSetTimestamp(
        'terminationDate',
