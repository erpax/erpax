...
const Shipments: CollectionConfig = {
  ...
  hooks: {
    ...
    beforeChange: [
      ...
      autoSetTimestamp('shippedAt', (d) => (d as { status?: string }).status === 'shipped'),
      autoSetTimestamp('deliveredAt', (d) => (d as { status?: string }).status === 'delivered'),
    ],
    afterChange: [auditTrailAfterChange('shipments')],
  },
  timestamps: true,
}
