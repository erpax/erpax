...
const WorkflowDefinitions: CollectionConfig = {
  ...
  fields: [
    ...
    {
      // The document lifecycle as a real state machine (states + transitions),
      // distinct from the approval `steps`. Grounded in each collection's actual
      // status enum; transitions carry the content-uuid `event` they emit, so
      // the independent machines synchronise into one whole (the merged horo).
      name: 'stateMachine',
      type: 'json',
      admin: { description: 'BPMN-2.0 state machine: { states[], transitions[{from,to,on,guard,emits}], documentChain[], events[], standards[] }. Drives the lifecycle gate + cross-domain event wiring.' },
    },
  ...
  hooks: standardCollectionHooks('workflow-definitions'),
  timestamps: true,
}
