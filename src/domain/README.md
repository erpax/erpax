<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# domain

> atom `domain` · horo `7` `descent` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `10` |  |
| Σ `17` | Σ `0` |

> [[balance]] `17` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `domain` · sub-root `f65c6e03-ce46-88d3-aa91-bc8a185f754d`
- bond degree `34` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `1857333c-a158-8cad-8f56-6750403a2b1d`
- bonds in `5` · out `5`
- trinity form·code·proof `1`·`0`·`0`
- links `34` / `34`
- folded `1` · escapes `0`

## typography graph

- partition `domain`
- partition root `f65c6e03-ce46-88d3-aa91-bc8a185f754d`
- bond degree `34`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `af824144-c70b-8292-9217-56eed89f48e3` · framed by typography partition `domain` bonds `34` · `pnpm readme` · `pnpm readme:check`</sub>
