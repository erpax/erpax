...
function std(x: readonly number[], m = mean(x)): number {
  ...
  return x.length ? algebraSqrt(s / x.length) : 0
}
...
export function detrend(x: readonly number[], window: number): number[] {
  const n = x.length
  const w = exactMax(1, exactFloor(window))
  const out: number[] = new Array(n)
  for (let i = 0; i < n; i++) {
    const a = exactMax(0, i - w)
    const b = exactMin(n, i + w + 1)
    let s = 0
...
function hann(x: readonly number[]): number[] {
  const n = x.length
  const out: number[] = new Array(n)
  for (let i = 0; i < n; i++) out[i] = x[i]! * (0.5 - 0.5 * algebraCos((2 * PI * i) / (n - 1)))
  return out
}
...
function powerAt(x: readonly number[], f: number, fps: number): number {
  ...
  for (let i = 0; i < x.length; i++) {
    re += x[i]! * algebraCos(w * i)
    im += x[i]! * algebraSin(w * i)
  }
  return re * re + im * im
}
...
function concentration(spec: readonly { f: number; p: number }[]): number {
  ...
  if (total <= 0) return 0
  let near = 0
  for (const s of spec) if (exactAbs(s.f - peak.f) <= PEAK_HALF) near += s.p
  return near / total
}
...
export function coherence(signal: readonly number[], fps = 30, lo = PULSE_LO, hi = PULSE_HI): number {
  ...
  const x = hann(detrend(signal, exactMax(1, exactRound(fps))))
...
export function extractPulse(trace: readonly number[], fps = 30): Pulse {
  ...
  const x = hann(detrend(trace, exactRound(fps)))
  ...
  ps.sort((a, b) => a - b)
  const median = ps[exactFloor(ps.length / 2)] || 1
