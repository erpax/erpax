...
//
// MERGED from the former index.mts twin (2026-07-27): the `.mts` extension had no
// webpack TS loader in `next build --webpack`, so every app-graph consumer of
// `@/corpus` (admin dashboards → monitor/aura/readme) failed to compile. One `.ts`
// source, no twin — the duplication that raised the question is gone.
...
// Navigation re-exports — the corpus barrel also surfaces the nav model (single `@/corpus` face).
export {
  adminGroupOf,
