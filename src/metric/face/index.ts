...
/** The formula, DECLARED — so a mismatch is read rather than guessed at. @see ./SKILL.md */
...
/** erpax's formula — a function, so it is not an exported constant ([[matrix]]/constants-audit). */
export const erpaxProtocol = (): FaceProtocol => ({
...
/** `different-convention` keeps a checker from accusing another formula of tampering. */
...
export function verifyFace(face: MetricFace): FaceVerdict {
  ...
  const declaredOther = face.protocol !== undefined && face.protocol.id !== erpaxProtocol().id
