...
export const ConsistencyAgent: DomainAgent = {
  ...
  async onSchedule(ctx: AgentContext): Promise<AgentEffect[]> {
    ...
    try {
      ...
      const { processInvariantResults, proposeFixFor } = await import('@/meta/automation')
      ...
      } = await import('@/agent/memory-writer')
      ...
      try {
        const { dryCleanScan } = await import('@/agents/mcp/dry-clean')
