...
2. **Densest tamper fuel.** A Lexical field is many nodes; **localized** ([[localize-field]]) it is a whole tree *per locale*. So one localized Lexical field contributes `locales × nodes` independent content-addresses to the [[tamper-cost]] surface — the richest increment of coverage of any field type. Localizing Lexical is the cheapest way to push the crack-cost toward ∞.

Each node is hashable, so the tree is a [[fractal]] Merkle surface and the whole field rolls into the row's content-[[uuid]] ([[identity]]). Matter-twin: the root config `editor` + the shared Lexical block set. Composes: [[config]] (editor) · [[localize]] · [[localize-field]] · [[tamper-cost]] · [[collapse]] · [[fields]] · [[whole]] · [[part]] · [[fractal]] · [[uuid]].
