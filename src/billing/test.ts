...
/** Pass the minimal mock where the handlers expect a real Payload instance. */
...
const createMockStripeSubscription = (overrides: Partial<Stripe.Subscription> = {}): Stripe.Subscription => ({
  ...
  created: exactFloor(Date.now() / 1000),
  currency: 'usd',
  customer: 'cus_123',
  current_period_end: exactFloor(Date.now() / 1000) + 30 * 24 * 60 * 60,
  current_period_start: exactFloor(Date.now() / 1000),
  ...
  pending_update: null,
  schedule: null,
  start_date: exactFloor(Date.now() / 1000),
  status: 'active',
  test_clock: null,
  transfer_data: null,
  trial_end: null,
...
const createMockStripeInvoice = (overrides: Partial<Stripe.Invoice> & Record<string, unknown> = {}): Stripe.Invoice => ({
  id: 'in_123',
  object: 'invoice',
  account_country: 'US',
  account_name: null,
    ...
    data: [
      {
        ...
        livemode: false,
        metadata: {},
        period: { end: exactFloor(Date.now() / 1000) + 30 * 24 * 60 * 60, start: exactFloor(Date.now() / 1000) },
        plan: null,
        price: {
          id: 'price_123',
          object: 'price',
  ...
  period_end: exactFloor(Date.now() / 1000),
  period_start: exactFloor(Date.now() / 1000) - 30 * 24 * 60 * 60,
...
describe('stripeWebhookHandlers', () => {
  ...
  describe('findPayloadSubscriptionByStripeId', () => {
    it('should find subscription by Stripe ID', async () => {
      ...
      const result = await findPayloadSubscriptionByStripeId(p(mockPayload), 'sub_123')
    ...
    it('should return null when not found', async () => {
      ...
      const result = await findPayloadSubscriptionByStripeId(p(mockPayload), 'sub_nonexistent')
  ...
  describe('updateSubscriptionStatus', () => {
    it('should update subscription status with logging', async () => {
      ...
      await updateSubscriptionStatus(p(mockPayload), 'sub-123', 'active', 'Test reason')
  ...
  describe('handleSubscriptionSync', () => {
    it('should create new Payload subscription from Stripe subscription', async () => {
      ...
      await handleSubscriptionSync(
        { event: {} as unknown as Stripe.Event, payload: p(mockPayload) },
        stripeSubscription,
      )
  ...
  describe('handleInvoicePaid', () => {
    it('should mark invoice as paid and update subscription', async () => {
      ...
      await handleInvoicePaid({ event: {} as unknown as Stripe.Event, payload: p(mockPayload) }, stripeInvoice)
