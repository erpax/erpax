/**
 * Tenant-Based Access Control — unified multi-tenant patterns.
 *
 * Consolidated from:
 * - src/plugins/accounting/middleware/tenant-scope.ts
 * - src/plugins/auth/access.ts
 *
 * @standard NIST INCITS-359-2012 role-based-access-control
 * @standard NIST SP-800-162 attribute-based-access-control
 * @security ISO-27001:2022 A.5.15 access-control
 * @security ISO-27001:2022 A.5.18 access-rights
 * @security ISO-27001:2022 A.5.23 cloud-service-tenant-isolation
 * @security ISO-27002:2022 §5.15 access-control
 * @security ISO-27002:2022 §5.4 segregation-of-duties
...
/**
 * Access result for query building
 */
 ...
 *
 * Derives `tenant` from the canonical `req.user.tenants[]` array
 * (multi-tenant plugin convention). Picks the first tenant; multi-tenant users
...
export function getUserContext(req: PayloadRequest): UserContext | null {
  ...
  const tenantsArr = (req.user as unknown as { tenants?: Array<{ tenant?: number | string }> })
    .tenants
...
/**
 * Validate that request tenant matches document tenant.
 * Used by services to ensure multi-tenant isolation.
 */
...
/**
 * Enrich a document with tenant context — automatically adds `tenant`
 */
export function enrichWithTenantContext<T extends Record<string, unknown>>(
  doc: T,
