 ...
 * wikiMap (.vitepress/corpus.mts) and the aura gate (aura/scan.ts) resolve [[links]]
 ...
 *
 *   pnpm harmony            # collection field + enum names
 *   pnpm harmony --json     # machine-readable (for the breath loop)
 */
...
function walk(dir: string, pred: WalkPredicate, out: string[] = []): string[] {
  let entries: string[]
  try {
...
const res: readonly RegExp[] = [/\bname:\s*'([A-Za-z][A-Za-z0-9_]*)'/g, /\bvalue:\s*'([A-Za-z][A-Za-z0-9_]*)'/g]
let total = 0
let harmonic = 0
const miss = new Map<string, number>()
const breaks: { name: string; bad: string }[] = []
