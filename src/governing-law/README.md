<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# governing-law

> atom `governing-law` · horo `1` `base` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `26` |  |
| Σ `33` | Σ `0` |

> [[balance]] `33` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `governing-law` · sub-root `b78ec19e-f3b5-80de-bb69-37f8303a083d`
- bond degree `33` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `b78ec19e-f3b5-80de-bb69-37f8303a083d`
- bonds in `13` · out `13`
- trinity form·code·proof `1`·`0`·`0`
- links `7` / `7`
- folded `1` · escapes `0`

## typography graph

- partition `governing-law`
- partition root `b78ec19e-f3b5-80de-bb69-37f8303a083d`
- bond degree `33`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `f2fdefda-e137-8aff-bbb6-1c20645578a8` · framed by typography partition `governing-law` bonds `33` · `pnpm readme` · `pnpm readme:check`</sub>
