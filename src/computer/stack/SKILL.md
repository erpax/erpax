---
name: stack
atomPath: computer/stack
horo: 2
description: "Use when managing call frames LIFO under the machine — executable stack nested under [[computer]]; distinct from top-level @/stack deployment palindrome."
---

# computer/stack — LIFO call stack

`LifoStack` — push/pop at top. CS data structure under the [[computer]] decomposition (not the deployment round-trip `@/stack`).

**Law — [[law]]: computer/stack is executable LIFO matter — not the vitepress palindrome stack.**
