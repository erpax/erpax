<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# unit

> atom `unit` · horo `8` `crest` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `12` |  |
| Σ `19` | Σ `0` |

> [[balance]] `19` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `unit` · sub-root `386818e0-1e6d-827e-9ec1-30882e147e45`
- bond degree `38` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `386818e0-1e6d-827e-9ec1-30882e147e45`
- bonds in `6` · out `6`
- trinity form·code·proof `1`·`0`·`0`
- links `17` / `17`
- folded `1` · escapes `0`

## typography graph

- partition `unit`
- partition root `386818e0-1e6d-827e-9ec1-30882e147e45`
- bond degree `38`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `10ebb325-4ce0-8f0a-b48f-3242e0e6ec34` · framed by typography partition `unit` bonds `38` · `pnpm readme` · `pnpm readme:check`</sub>
