<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# decide

> atom `decide` · horo `8` `crest` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[trinity]]/code `1` |  |
| [[asset]]/[[trinity]]/proof `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `8` |  |
| Σ `17` | Σ `0` |

> [[balance]] `17` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `decide` · sub-root `e5c5c062-faef-8e53-b163-f815f5353b77`
- bond degree `26` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `e5c5c062-faef-8e53-b163-f815f5353b77`
- bonds in `4` · out `4`
- trinity form·code·proof `1`·`1`·`1`
- links `26` / `26`
- folded `1` · escapes `0`

## typography graph

- partition `decide`
- partition root `e5c5c062-faef-8e53-b163-f815f5353b77`
- bond degree `26`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `25af3a58-295c-8eb5-89e5-aaed986f0647` · framed by typography partition `decide` bonds `26` · `pnpm readme` · `pnpm readme:check`</sub>
