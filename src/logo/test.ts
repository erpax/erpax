...
// possible state of logo and forbade nothing. What logo actually owes its
// callers is its FACE: import { X } from '@/logo' breaks the moment the barrel
// stops offering X, and nothing else in the tree reports that.
const OFFERED = [
  "Logo"
] as const

describe('logo — the face it owes its callers', () => {
  it('offers every name a caller may import', () => {
    const live = new Set(faceOf('logo'))
    const lost = OFFERED.filter((n) => !live.has(n))
    expect(lost).toEqual([])
  })

  it('offers 1 name(s) — a silent drop changes the count', () => {
    expect(faceOf('logo').length).toBeGreaterThanOrEqual(OFFERED.length)
  })

  it('names nothing twice — a duplicated export is an ambiguous import', () => {
    const live = faceOf('logo')
