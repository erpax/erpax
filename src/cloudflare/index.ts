...
export function makeMediator(ctx: MediatorContext): {
  ...
  return {
    ...
    signUuid: async (uuid, kid) => {
      const { getDefaultKeyResolver } = await import('@/integrity/tenant-key-registry')
      const { signContentUuid } = await import('@/integrity/signatures')
    ...
    verifyUuid: async (signed) => {
      const { getDefaultKeyResolver } = await import('@/integrity/tenant-key-registry')
      const { verifyContentUuidSignature } = await import('@/integrity/signatures')
      const resolver = getDefaultKeyResolver()
    ...
    encryptEnvelope: async (plaintext, uuid, kid) => {
      const { getDefaultKeyResolver } = await import('@/integrity/tenant-key-registry')
      const { encryptEnvelope: enc } = await import('@/integrity/envelope')
      const resolver = getDefaultKeyResolver()
    ...
    decryptEnvelope: async (envelope) => {
      const { getDefaultKeyResolver } = await import('@/integrity/tenant-key-registry')
      const { decryptEnvelope: dec } = await import('@/integrity/envelope')
      const resolver = getDefaultKeyResolver()
    ...
    runQuery: async (args) => {
      const { runWithFingerprint } = await import('@/query/fingerprint')
