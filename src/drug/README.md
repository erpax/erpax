<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# drug

> atom `drug` · horo `5` `round` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `16` |  |
| Σ `23` | Σ `0` |

> [[balance]] `23` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `drug` · sub-root `6e036fb0-6ddc-87d6-ade9-afd1cb981d51`
- bond degree `44` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `6e036fb0-6ddc-87d6-ade9-afd1cb981d51`
- bonds in `8` · out `8`
- trinity form·code·proof `1`·`0`·`0`
- links `16` / `16`
- folded `1` · escapes `0`

## typography graph

- partition `drug`
- partition root `6e036fb0-6ddc-87d6-ade9-afd1cb981d51`
- bond degree `44`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `b81d6c88-1de1-8e2e-a487-a373d5f0e80c` · framed by typography partition `drug` bonds `44` · `pnpm readme` · `pnpm readme:check`</sub>
