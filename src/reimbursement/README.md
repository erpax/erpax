<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# reimbursement

> atom `reimbursement` · horo `7` `descent` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `24` |  |
| Σ `31` | Σ `0` |

> [[balance]] `31` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `reimbursement` · sub-root `a5919c12-8264-8875-8bc6-c93a5643faa8`
- bond degree `28` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `a5919c12-8264-8875-8bc6-c93a5643faa8`
- bonds in `12` · out `12`
- trinity form·code·proof `1`·`0`·`0`
- links `4` / `4`
- folded `1` · escapes `0`

## typography graph

- partition `reimbursement`
- partition root `a5919c12-8264-8875-8bc6-c93a5643faa8`
- bond degree `28`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `bce1a067-7808-8eac-8508-53e2b9d2b948` · framed by typography partition `reimbursement` bonds `28` · `pnpm readme` · `pnpm readme:check`</sub>
