<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# toxicologic

> atom `toxicologic` · horo `4` `weave` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `4` |  |
| Σ `11` | Σ `0` |

> [[balance]] `11` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `toxicologic` · sub-root `35ff3237-3a16-8032-9f84-f1df836d44c8`
- bond degree `8` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `35ff3237-3a16-8032-9f84-f1df836d44c8`
- bonds in `2` · out `2`
- trinity form·code·proof `1`·`0`·`0`
- links `4` / `4`
- folded `1` · escapes `0`

## typography graph

- partition `toxicologic`
- partition root `35ff3237-3a16-8032-9f84-f1df836d44c8`
- bond degree `8`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `fc786655-8faa-8da5-94b6-52f4bb3f37f2` · framed by typography partition `toxicologic` bonds `8` · `pnpm readme` · `pnpm readme:check`</sub>
