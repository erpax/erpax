<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# forestry

> atom `forestry` · horo `7` `descent` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `8` |  |
| Σ `15` | Σ `0` |

> [[balance]] `15` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `forestry` · sub-root `c3742c7f-011a-853c-b908-2a6616f77fe1`
- bond degree `27` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `c3742c7f-011a-853c-b908-2a6616f77fe1`
- bonds in `4` · out `4`
- trinity form·code·proof `1`·`0`·`0`
- links `33` / `33`
- folded `1` · escapes `0`

## typography graph

- partition `forestry`
- partition root `c3742c7f-011a-853c-b908-2a6616f77fe1`
- bond degree `27`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `780176f9-4529-85c6-9f64-976171252f84` · framed by typography partition `forestry` bonds `27` · `pnpm readme` · `pnpm readme:check`</sub>
