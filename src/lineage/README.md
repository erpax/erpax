<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# lineage

> atom `lineage` · horo `1` `base` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `12` |  |
| Σ `19` | Σ `0` |

> [[balance]] `19` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `lineage` · sub-root `d2716b54-26bb-878e-bd2f-be75d6687051`
- bond degree `46` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `d2716b54-26bb-878e-bd2f-be75d6687051`
- bonds in `6` · out `6`
- trinity form·code·proof `1`·`0`·`0`
- links `24` / `24`
- folded `1` · escapes `0`

## typography graph

- partition `lineage`
- partition root `d2716b54-26bb-878e-bd2f-be75d6687051`
- bond degree `46`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `3131a1ed-85c8-8c54-b2b4-40559c8daab9` · framed by typography partition `lineage` bonds `46` · `pnpm readme` · `pnpm readme:check`</sub>
