...
import { emitRoutingCompleted } from '@/chain/event/emitter'
...
const Routings: CollectionConfig = {
  ...
  fields: [
    ...
    unitOfMeasureField({ description: 'UoM the run time is per (C62/KGM/LTR/MTR…) — discrete and process.' }),
    statusField(
      [
  ...
  hooks: standardCollectionHooks('routings', { afterChange: [emitRoutingCompleted] }),
  timestamps: true,
}
