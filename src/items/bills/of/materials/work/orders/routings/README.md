<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# routings

> atom `items/bills/of/materials/work/orders/routings` · horo `7` `descent` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[trinity]]/code `1` |  |
| [[asset]]/[[trinity]]/proof `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `8` |  |
| Σ `17` | Σ `0` |

> [[balance]] `17` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `items` · sub-root `1341dba3-9395-8200-8e47-ea3dc4d4b9e9`
- bond degree `15` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `b7bb1407-4054-8648-8d1f-3bd24613d051`
- bonds in `4` · out `4`
- trinity form·code·proof `1`·`1`·`1`
- links `8` / `8`
- folded `1` · escapes `0`

## typography graph

- partition `items`
- partition root `1341dba3-9395-8200-8e47-ea3dc4d4b9e9`
- bond degree `15`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `fe82dd55-ca42-8cfd-a939-be4ac0fd9d59` · framed by typography partition `items` bonds `15` · `pnpm readme` · `pnpm readme:check`</sub>
