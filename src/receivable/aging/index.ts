/** @index-cross.foldback child=receivable/aging parent=receivable — this cross folds back into its parent. */
...
export class ARAgingCalculator {
  ...
  static calculateCollectionPriority(
    ...
    return invoices
      .map((inv) => ({
        invoiceId: inv.id,
        priority: exactMax(0, daysBetween(inv.dueDate, new Date())) * (inv.balance / 10000),
      }))
