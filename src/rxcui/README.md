<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# rxcui

> atom `rxcui` · horo `4` `weave` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `8` |  |
| Σ `15` | Σ `0` |

> [[balance]] `15` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `rxcui` · sub-root `48d39c2d-aa5a-8dde-9a36-c00e4273263e`
- bond degree `10` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `48d39c2d-aa5a-8dde-9a36-c00e4273263e`
- bonds in `4` · out `4`
- trinity form·code·proof `1`·`0`·`0`
- links `2` / `2`
- folded `1` · escapes `0`

## typography graph

- partition `rxcui`
- partition root `48d39c2d-aa5a-8dde-9a36-c00e4273263e`
- bond degree `10`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `0ba34f50-efcc-8f00-bff3-2c71d87d8d15` · framed by typography partition `rxcui` bonds `10` · `pnpm readme` · `pnpm readme:check`</sub>
