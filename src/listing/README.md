<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# listing

> atom `listing` · horo `7` `descent` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `10` |  |
| Σ `17` | Σ `0` |

> [[balance]] `17` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `listing` · sub-root `1ae4ceaf-ae89-833e-83a5-b9fb10db06dc`
- bond degree `18` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `1ae4ceaf-ae89-833e-83a5-b9fb10db06dc`
- bonds in `5` · out `5`
- trinity form·code·proof `1`·`0`·`0`
- links `6` / `6`
- folded `1` · escapes `0`

## typography graph

- partition `listing`
- partition root `1ae4ceaf-ae89-833e-83a5-b9fb10db06dc`
- bond degree `18`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `69800b5b-29b2-8a21-a8af-fc7a7758b3c9` · framed by typography partition `listing` bonds `18` · `pnpm readme` · `pnpm readme:check`</sub>
