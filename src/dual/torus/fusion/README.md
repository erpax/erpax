<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# fusion

> atom `dual/torus/fusion` · horo `5` `round` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[trinity]]/code `1` |  |
| [[asset]]/[[trinity]]/proof `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `16` |  |
| Σ `25` | Σ `0` |

> [[balance]] `25` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `dual` · sub-root `7e30f964-ac4d-8ec7-8f27-6deddfb3ca2d`
- bond degree `59` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `4c828c05-935d-84a4-96e4-52f0bd340316`
- bonds in `8` · out `8`
- trinity form·code·proof `1`·`1`·`1`
- links `9` / `9`
- folded `1` · escapes `0`

## typography graph

- partition `dual`
- partition root `7e30f964-ac4d-8ec7-8f27-6deddfb3ca2d`
- bond degree `59`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `0fab9d0a-e574-8524-8a1a-63a011284a50` · framed by typography partition `dual` bonds `59` · `pnpm readme` · `pnpm readme:check`</sub>
