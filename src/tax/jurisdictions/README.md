<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# jurisdictions

> atom `tax/jurisdictions` · horo `4` `weave` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[trinity]]/code `1` |  |
| [[asset]]/[[trinity]]/proof `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `10` |  |
| Σ `19` | Σ `0` |

> [[balance]] `19` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `tax` · sub-root `94db23f9-1cc9-8309-92c8-a21dfe445e59`
- bond degree `18` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `510867d8-1738-8556-ae7e-c895f3a6d9eb`
- bonds in `5` · out `5`
- trinity form·code·proof `1`·`1`·`1`
- links `3` / `3`
- folded `1` · escapes `0`

## typography graph

- partition `tax`
- partition root `94db23f9-1cc9-8309-92c8-a21dfe445e59`
- bond degree `18`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `19ab6d7e-9a53-8179-a208-d0d271c7ae0c` · framed by typography partition `tax` bonds `18` · `pnpm readme` · `pnpm readme:check`</sub>
