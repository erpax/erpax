...
import { deriveOrientSection, skillDescriptionOf } from './derive-prose'
...
export function renderReadme(
    ...
    models && models.length > 0
      ? (() => {
          ...
          return {
            ...
            tables: stats.tables.filter((t) => t.axis === 'seal' || t.axis === 'balance'),
          }
        })()
      : null
  L.push(
  ...
  if (sealPivot) {
    L.push('', renderPivotMarkdown(sealPivot))
  }
  // Optional catalog faces — only when explicitly supplied (tests / CLI --index).
  if (sourceIndex && sourceIndex.length > 0) {
    L.push('', ...sourceIndex)
  }
  const entropyOpts: CorpusEntropyRenderOpts = {
    ...(entropyRender ?? {
  ...
  L.push(
    ...
    '',
    `<sub>${citation({ path: 'src', uuid })}</sub>`,
    '',
  )
  return L.join('\n')
}
 ...
 * Root omits the source-index catalog by default (QPU/FTL face); pass via renderReadme for catalogs.
 */
...
export function mergeCorpusAnalytics(a: CorpusAnalytics, b: CorpusAnalytics): CorpusAnalytics {
  ...
  return {
    ...
    meanBondDegree: folderCount > 0 ? exactRound((bondSum * 100) / folderCount) / 100 : 0,
...
export function applyFolderGravityGate(
  ...
  if (!membershipOk) purityVariance += exactMax(1, membershipViolationCount)
...
export function renderFolderReadme(model: FolderReadmeModel): string {
  ...
  const rows = exactMax(statement.debits.length, statement.credits.length)
  const L: string[] = [
  ...
  L.push(
    ...
    '',
    // Every generated page REPRODUCES corpus matter, and CC-BY-NC-ND §3(a)(1) wants attribution
    // with the source link on each reproduction. 6,943 pages carried none; this is computed from
    // the licence face, so it cannot drift and nobody types it. @see [[algebra]]/license
    `<sub>${citation({ path: `src/${model.atomPath}`, uuid })}</sub>`,
    '',
  )
  return L.join('\n')
}
...
export function rosettaMath(cwd: string = process.cwd(), depth = 6): RosettaMath {
  ...
  for (let k = 1; k <= depth; k++) generative.push({ leaves: k, messages: algebraFloatPow(basis, k) * catalan(k - 1) })
...
export function materializeComputedFacesInWaves(
  ...
  const policy = maxWorkTamperPolicy()
  for (const wave of corpusPathWaveBatches({}, policy)) {
    // Sub-chunk each wave so the memory unit is bounded and progress persists finely. A full 441-atom wave
    // materialised in one call OOMs (OS-killed, silent — no JS heap trace), and because nothing sealed before
    // the crash, every rerun died at the same atom: no convergence. Now each CHUNK materialises, SEALS the
    // ledger, then releases — so a crash loses at most one chunk, and a rerun reads the sealed ledger and skips
    // (READ, not re-derive) every atom already done, converging monotonically. This fits MEMORY exactly as the
    // timeout ladder fits TIME: split into units that complete + seal + resume, never one giant pass.
    const CHUNK = 48
    for (let i = 0; i < wave.items.length; i += CHUNK) {
      total += materializeComputedFacesForPaths(wave.items.slice(i, i + CHUNK), cwd, graph, ctx, ledger)
