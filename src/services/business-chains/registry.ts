...
export const BUSINESS_CHAINS: BusinessChainRegistry = {
  ...
  P2P_THREE_WAY_MATCH: {
    ...
    steps: [
      { collection: 'purchase-requisitions', action: 'submit',   emits: 'pr:submitted',   requires: [] },
      { collection: 'purchase-requisitions', action: 'approve',  emits: 'pr:approved',    requires: ['pr:submitted'] },
      { collection: 'vendor-quotes',         action: 'receive',  emits: 'rfq:received',   requires: ['pr:approved'] },
      { collection: 'vendor-quotes',         action: 'award',    emits: 'rfq:awarded',    requires: ['rfq:received'] },
      { collection: 'purchase-orders',       action: 'create',   emits: 'po:created',     requires: ['rfq:awarded'] },
      { collection: 'goods-receipts',        action: 'post',     emits: 'gr:posted',      requires: ['po:created'] },
      { collection: 'invoices',              action: 'activate', emits: 'bill:activated', requires: ['gr:posted'] },
      { collection: 'invoices',              action: 'three-way-match', emits: 'bill:matched', requires: ['bill:activated', 'gr:posted', 'po:created'] },
      { collection: 'payments',              action: 'send',     emits: 'payment:sent',   requires: ['bill:matched'] },
      { collection: 'invoices',              action: 'mark-paid', emits: 'bill:paid',     requires: ['payment:sent'] },
    ],
  ...
  IFRS16_LEASE_CYCLE: {
    ...
    featureGate: 'leasing',
    steps: [
      { collection: 'leases',                action: 'commence', emits: 'lease:commenced',     requires: [] },
      { collection: 'lease-period-postings', action: 'post',     emits: 'lpp:posted',          requires: ['lease:commenced'] },
      { collection: 'lease-modifications',   action: 'remeasure', emits: 'lease:remeasured',   requires: ['lease:commenced'] },
      { collection: 'lease-period-postings', action: 'post-after-mod', emits: 'lpp:posted',    requires: ['lease:remeasured'] },
      { collection: 'leases',                action: 'terminate', emits: 'lease:terminated',   requires: ['lease:commenced'] },
    ],
  ...
  SUBSCRIPTION_BILLING_CYCLE: {
    ...
    featureGate: undefined, // core
    steps: [
      { collection: 'subscriptions',     action: 'activate',  emits: 'subscription:activated', requires: [] },
      { collection: 'usage-records',     action: 'record',    emits: 'usage:recorded',         requires: ['subscription:activated'] },
      { collection: 'invoices',          action: 'issue',     emits: 'subscription:invoiced',  requires: ['usage:recorded'] },
      { collection: 'payments',          action: 'receive',   emits: 'payment:received',       requires: ['subscription:invoiced'] },
      { collection: 'invoices',          action: 'mark-paid', emits: 'invoice:completed',      requires: ['payment:received'] },
    ],
  ...
  H2R_HIRE_TO_RETIRE: {
    ...
    steps: [
      { collection: 'job-positions',         action: 'open',     emits: 'position:opened',   requires: [] },
      { collection: 'recruiting-pipeline',   action: 'apply',    emits: 'pipeline:applied',  requires: ['position:opened'] },
      { collection: 'recruiting-pipeline',   action: 'hire',     emits: 'pipeline:hired',    requires: ['pipeline:applied'] },
      { collection: 'employees',             action: 'create',   emits: 'employee:created',  requires: ['pipeline:hired'] },
      { collection: 'time-entries',          action: 'post',     emits: 'time:posted',       requires: ['employee:created'] },
      { collection: 'expense-reports',       action: 'approve',  emits: 'expense:approved',  requires: ['employee:created'] },
      { collection: 'leave-requests',        action: 'approve',  emits: 'leave:approved',    requires: ['employee:created'] },
      { collection: 'performance-reviews',   action: 'finalise', emits: 'review:finalised',  requires: ['employee:created'] },
      { collection: 'payroll-runs',          action: 'run',      emits: 'payroll:run',       requires: ['time:posted', 'expense:approved'] },
    ],
  ...
  MANUFACTURING_CYCLE: {
    ...
    steps: [
      { collection: 'bills-of-materials',   action: 'release',  emits: 'bom:released',    requires: [] },
      { collection: 'work-orders',          action: 'release',  emits: 'wo:released',     requires: ['bom:released'] },
      { collection: 'inventory-movements',  action: 'issue',    emits: 'inventory:issued', requires: ['wo:released'] },
      { collection: 'production-receipts',  action: 'post',     emits: 'prod:completed',  requires: ['inventory:issued'] },
      { collection: 'cost-variances',       action: 'compute',  emits: 'variance:computed', requires: ['prod:completed'] },
      { collection: 'quality-inspections',  action: 'inspect',  emits: 'qc:complete',     requires: ['prod:completed'] },
    ],
  ...
  WORKFLOW_APPROVAL_CYCLE: {
    ...
    steps: [
      ...
      { collection: 'workflow-instances',   action: 'spawn',          emits: 'workflow:spawned',         requires: [] },
      { collection: 'workflow-instances',   action: 'step-decision',  emits: 'workflow:step-decided',    requires: ['workflow:spawned'] },
      { collection: 'workflow-instances',   action: 'finalise',       emits: 'workflow:finalised',       requires: ['workflow:step-decided'] },
    ],
  ...
  BULK_IMPORT_CYCLE: {
    ...
    featureGate: undefined, // core
    steps: [
      { collection: 'transaction-failures',  action: 'enqueue', emits: 'bulk:enqueued',  requires: [] },
      { collection: 'transaction-failures',  action: 'process', emits: 'bulk:processed', requires: ['bulk:enqueued'] },
      { collection: 'transaction-failures',  action: 'reprocess', emits: 'bulk:reprocessed', requires: ['bulk:processed'] },
    ],
  ...
  O2C_GOODS: {
    ...
    featureGate: undefined, // core
    steps: [
      { collection: 'leads',                  action: 'qualify',     emits: 'lead:qualified',    requires: [] },
      { collection: 'opportunities',          action: 'close-won',   emits: 'opp:won',           requires: ['lead:qualified'] },
      { collection: 'quotes',                 action: 'send',        emits: 'quote:sent',        requires: ['opp:won'] },
      { collection: 'contracts',              action: 'sign',        emits: 'contract:signed',   requires: ['quote:sent'] },
      { collection: 'shipments',              action: 'dispatch',    emits: 'shipment:dispatched', requires: ['contract:signed'] },
      { collection: 'tracking-events',        action: 'deliver',     emits: 'shipment:delivered', requires: ['shipment:dispatched'] },
      { collection: 'invoices',               action: 'activate',    emits: 'invoice:activated', requires: ['shipment:delivered'] },
      { collection: 'payments',               action: 'receive',     emits: 'payment:received',  requires: ['invoice:activated'] },
      { collection: 'invoices',               action: 'complete',    emits: 'invoice:completed', requires: ['payment:received'] },
    ],
  ...
  O2C_SERVICES_OVER_TIME: {
    ...
    steps: [
      { collection: 'opportunities',           action: 'close-won',  emits: 'opp:won',           requires: [] },
      { collection: 'contracts',               action: 'sign',       emits: 'contract:signed',   requires: ['opp:won'] },
      { collection: 'performance-obligations', action: 'identify',   emits: 'po-obl:identified', requires: ['contract:signed'] },
      { collection: 'projects',                action: 'create',     emits: 'project:created',   requires: ['po-obl:identified'] },
      { collection: 'project-tasks',           action: 'plan',       emits: 'task:planned',      requires: ['project:created'] },
      { collection: 'time-entries',            action: 'post',       emits: 'time:posted',       requires: ['task:planned'] },
      { collection: 'wip-snapshots',           action: 'snapshot',   emits: 'wip:snapshot:posted', requires: ['time:posted'] },
      { collection: 'project-milestones',      action: 'achieve',    emits: 'milestone:achieved', requires: ['wip:snapshot:posted'] },
      { collection: 'invoices',                action: 'milestone-invoice', emits: 'invoice:activated', requires: ['milestone:achieved'] },
      { collection: 'payments',                action: 'receive',    emits: 'payment:received',  requires: ['invoice:activated'] },
    ],
  ...
  CRM_LEAD_TO_CASH: {
    ...
    featureGate: 'crm',
    steps: [
      { collection: 'leads',         action: 'create',       emits: 'lead:created',      requires: [] },
      { collection: 'activities',    action: 'log',          emits: 'activity:logged',   requires: ['lead:created'] },
      { collection: 'leads',         action: 'qualify-mql',  emits: 'lead:mql',          requires: ['activity:logged'] },
      { collection: 'leads',         action: 'qualify-sql',  emits: 'lead:sql',          requires: ['lead:mql'] },
      { collection: 'opportunities', action: 'create',       emits: 'opp:created',       requires: ['lead:sql'] },
      { collection: 'opportunities', action: 'close-won',    emits: 'opp:won',           requires: ['opp:created'] },
      { collection: 'customers',     action: 'create',       emits: 'customer:created',  requires: ['opp:won'] },
      { collection: 'contracts',     action: 'sign',         emits: 'contract:signed',   requires: ['customer:created'] },
      { collection: 'sales-commissions', action: 'book',     emits: 'commission:booked', requires: ['opp:won'] },
    ],
  ...
  PROVISION_LIFECYCLE: {
    ...
    featureGate: 'period_end_closing',
    steps: [
      { collection: 'audit-findings',  action: 'recognise',  emits: 'finding:recognised', requires: [] },
      { collection: 'provisions',      action: 'recognise',  emits: 'provision:recognised', requires: ['finding:recognised'] },
      { collection: 'provisions',      action: 'remeasure',  emits: 'provision:remeasured', requires: ['provision:recognised'] },
      { collection: 'provisions',      action: 'use',        emits: 'provision:used',     requires: ['provision:recognised'] },
      { collection: 'provisions',      action: 'reverse',    emits: 'provision:reversed', requires: ['provision:recognised'] },
    ],
  ...
  ESG_REPORTING_CYCLE: {
    ...
    steps: [
      { collection: 'carbon-emissions',     action: 'record',     emits: 'ghg:recorded',      requires: [] },
      { collection: 'csrd-disclosures',     action: 'rollup',     emits: 'csrd:rolled-up',    requires: ['ghg:recorded'] },
      { collection: 'csrd-disclosures',     action: 'assure',     emits: 'csrd:assured',      requires: ['csrd:rolled-up'] },
      { collection: 'evidence-attestations', action: 'sign',      emits: 'evidence:signed',   requires: ['csrd:assured'] },
      { collection: 'csrd-disclosures',     action: 'file',       emits: 'csrd:filed',        requires: ['evidence:signed'] },
    ],
  ...
  KYC_SANCTIONS_REVIEW: {
    ...
    steps: [
      { collection: 'customers',         action: 'onboard',      emits: 'customer:onboard',     requires: [] },
      { collection: 'kyc-checks',        action: 'initiate',     emits: 'kyc:initiated',        requires: ['customer:onboard'] },
      { collection: 'beneficial-owners', action: 'register',     emits: 'ubo:registered',       requires: ['kyc:initiated'] },
      { collection: 'ai-suggestions',    action: 'screen',       emits: 'sanctions:screened',   requires: ['ubo:registered'] },
      { collection: 'kyc-checks',        action: 'approve',      emits: 'kyc:approved',         requires: ['sanctions:screened'] },
      { collection: 'kyc-checks',        action: 'rescreen',     emits: 'kyc:rescreened',       requires: ['kyc:approved'] },
    ],
  ...
  MULTI_INVOICE_PAYMENT_ALLOCATION: {
    ...
    featureGate: undefined, // core
    steps: [
      { collection: 'invoices',            action: 'activate-1', emits: 'invoice:activated', requires: [] },
      { collection: 'invoices',            action: 'activate-2', emits: 'invoice:activated', requires: [] },
      { collection: 'invoices',            action: 'activate-3', emits: 'invoice:activated', requires: [] },
      { collection: 'payments',            action: 'receive-bulk', emits: 'payment:received', requires: ['invoice:activated'] },
      { collection: 'payment-allocations', action: 'allocate-1', emits: 'allocation:posted', requires: ['payment:received'] },
      { collection: 'payment-allocations', action: 'allocate-2', emits: 'allocation:posted', requires: ['payment:received'] },
      { collection: 'payment-allocations', action: 'allocate-3', emits: 'allocation:posted', requires: ['payment:received'] },
      { collection: 'invoices',            action: 'complete-1', emits: 'invoice:completed', requires: ['allocation:posted'] },
      { collection: 'invoices',            action: 'complete-2', emits: 'invoice:completed', requires: ['allocation:posted'] },
      { collection: 'invoices',            action: 'complete-3', emits: 'invoice:completed', requires: ['allocation:posted'] },
    ],
  ...
  INTERCOMPANY_CONSOLIDATION: {
    ...
    steps: [
      { collection: 'legal-entities',              action: 'register-sub-a',  emits: 'entity:registered', requires: [] },
      { collection: 'legal-entities',              action: 'register-sub-b',  emits: 'entity:registered', requires: [] },
      { collection: 'intercompany-transactions',   action: 'post-leg-a',      emits: 'ic:posted', requires: ['entity:registered'] },
      { collection: 'intercompany-transactions',   action: 'post-leg-b',      emits: 'ic:posted', requires: ['entity:registered'] },
      { collection: 'intercompany-transactions',   action: 'reconcile-pair',  emits: 'ic:reconciled', requires: ['ic:posted'] },
  ...
  MULTI_VENDOR_PR_SPLIT_AWARD: {
    ...
    steps: [
      { collection: 'purchase-requisitions', action: 'submit',       emits: 'pr:submitted', requires: [] },
      { collection: 'purchase-requisitions', action: 'approve',      emits: 'pr:approved',  requires: ['pr:submitted'] },
      { collection: 'vendor-quotes',         action: 'receive-q1',   emits: 'rfq:received', requires: ['pr:approved'] },
      { collection: 'vendor-quotes',         action: 'receive-q2',   emits: 'rfq:received', requires: ['pr:approved'] },
      { collection: 'vendor-quotes',         action: 'receive-q3',   emits: 'rfq:received', requires: ['pr:approved'] },
      { collection: 'vendor-quotes',         action: 'award-q1',     emits: 'rfq:awarded',  requires: ['rfq:received'] },
      { collection: 'vendor-quotes',         action: 'award-q2',     emits: 'rfq:awarded',  requires: ['rfq:received'] },
      { collection: 'purchase-orders',       action: 'create-po-1',  emits: 'po:created',   requires: ['rfq:awarded'] },
      { collection: 'purchase-orders',       action: 'create-po-2',  emits: 'po:created',   requires: ['rfq:awarded'] },
    ],
  ...
  NOTIFICATION_DISPATCH: {
    ...
    featureGate: undefined, // core
    steps: [
      { collection: 'audit-events',  action: 'observe-domain-event', emits: 'notify:observed', requires: [] },
      { collection: 'audit-events',  action: 'dispatch',             emits: 'notify:dispatched', requires: ['notify:observed'] },
    ],
  ...
  CONSIGNMENT_CYCLE: {
    ...
    steps: [
      { collection: 'consignment-arrangements', action: 'sign',           emits: 'consignment:arranged', requires: [] },
      { collection: 'inventory-movements',      action: 'ship-to-consignee', emits: 'inventory:issued',  requires: ['consignment:arranged'] },
      { collection: 'consignment-inventory',    action: 'open-balance',   emits: 'consignment:on-hand', requires: ['inventory:issued'] },
      { collection: 'consignment-sales',        action: 'report-sale',    emits: 'consignment:sold',    requires: ['consignment:on-hand'] },
      { collection: 'invoices',                 action: 'invoice-consignee', emits: 'invoice:activated', requires: ['consignment:sold'] },
      { collection: 'payments',                 action: 'receive-payment', emits: 'payment:received',   requires: ['invoice:activated'] },
    ],
  ...
  RESOURCE_BOOKING_CYCLE: {
    ...
    steps: [
      { collection: 'bookable-resources', action: 'catalogue',  emits: 'resource:catalogued', requires: [] },
      { collection: 'bookings',           action: 'request',    emits: 'booking:requested',   requires: ['resource:catalogued'] },
      { collection: 'bookings',           action: 'confirm',    emits: 'booking:confirmed',   requires: ['booking:requested'] },
      { collection: 'bookings',           action: 'check-in',   emits: 'booking:checked_in',  requires: ['booking:confirmed'] },
      { collection: 'bookings',           action: 'check-out',  emits: 'booking:checked_out', requires: ['booking:checked_in'] },
      { collection: 'invoices',           action: 'invoice',    emits: 'invoice:activated',   requires: ['booking:checked_out'] },
      { collection: 'payments',           action: 'pay',        emits: 'payment:received',    requires: ['invoice:activated'] },
    ],
  ...
  FACILITY_MAINTENANCE_CYCLE: {
    ...
    steps: [
      { collection: 'properties',               action: 'register-property', emits: 'property:registered', requires: [] },
      { collection: 'spaces',                   action: 'register-space',    emits: 'space:registered',    requires: ['property:registered'] },
      { collection: 'maintenance-requests',     action: 'raise',             emits: 'mr:raised',           requires: ['space:registered'] },
      { collection: 'maintenance-requests',     action: 'triage',            emits: 'mr:triaged',          requires: ['mr:raised'] },
      { collection: 'maintenance-work-orders',  action: 'issue',             emits: 'wo:issued',           requires: ['mr:triaged'] },
      { collection: 'inventory-movements',      action: 'issue-parts',       emits: 'inventory:issued',    requires: ['wo:issued'] },
      { collection: 'time-entries',             action: 'book-labour',       emits: 'time:posted',         requires: ['wo:issued'] },
      { collection: 'maintenance-work-orders',  action: 'complete',          emits: 'wo:completed',        requires: ['inventory:issued', 'time:posted'] },
      { collection: 'quality-inspections',      action: 'inspect',           emits: 'qc:complete',         requires: ['wo:completed'] },
      { collection: 'maintenance-work-orders',  action: 'close-and-post',    emits: 'wo:closed',           requires: ['qc:complete'] },
    ],
