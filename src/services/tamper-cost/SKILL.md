...
Matter-twin: `tamper-cost/index.ts` (`crackVerdict`/`secondPreimageLog2`/`birthdayMarginBits`) + `index.test.ts` (the proof — green by construction). The cited strengths must be true ([[standard]] NIST SP 800-107, RFC 9562 §8). Composes: [[uuid]] · [[identity]] · [[merge]] · [[aura]] · [[holographic]] · [[whole]] · [[one]] · [[collapse]] · [[standard]].
