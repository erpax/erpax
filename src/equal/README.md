<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# equal

> atom `equal` · horo `4` `weave` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `6` |  |
| Σ `13` | Σ `0` |

> [[balance]] `13` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `equal` · sub-root `b9a1505b-5ec5-8fde-ab39-2697c5e7d0e6`
- bond degree `13` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `b9a1505b-5ec5-8fde-ab39-2697c5e7d0e6`
- bonds in `3` · out `3`
- trinity form·code·proof `1`·`0`·`0`
- links `4` / `4`
- folded `1` · escapes `0`

## typography graph

- partition `equal`
- partition root `b9a1505b-5ec5-8fde-ab39-2697c5e7d0e6`
- bond degree `13`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `133741b3-db00-83c8-bd98-06fd54eee633` · framed by typography partition `equal` bonds `13` · `pnpm readme` · `pnpm readme:check`</sub>
