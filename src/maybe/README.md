<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# maybe

> atom `maybe` · horo `2` `share` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `16` |  |
| Σ `23` | Σ `0` |

> [[balance]] `23` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `maybe` · sub-root `c3c16bda-96de-87e2-91c1-8e180fe0c2b4`
- bond degree `20` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `c3c16bda-96de-87e2-91c1-8e180fe0c2b4`
- bonds in `8` · out `8`
- trinity form·code·proof `1`·`0`·`0`
- links `4` / `4`
- folded `1` · escapes `0`

## typography graph

- partition `maybe`
- partition root `c3c16bda-96de-87e2-91c1-8e180fe0c2b4`
- bond degree `20`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `00cb35cf-ce16-8bb5-acb2-408a2ad3aa0f` · framed by typography partition `maybe` bonds `20` · `pnpm readme` · `pnpm readme:check`</sub>
