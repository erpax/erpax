/**
 * Multi-tenant boundary tests — config smoke (always) + optional D1 integration.
 *
 * Pre-push / CI set `PAYLOAD_TEST_SKIP_MIGRATE=1`, so the D1-backed sandbox
 * suite is skipped; the config smoke pins still prove multi-tenant wiring.
 * Run vitest without that env (migrate in vitest.setup) for full CRUD proof.
 *
...
const skipDbIntegration = Boolean(process.env.PAYLOAD_TEST_SKIP_MIGRATE)
...
describe('payload.config — multi-tenant wiring (config smoke)', () => {
  it('config resolves and registers tenant-scoped collections', async () => {
    const payloadConfig = await config
    const slugs = new Set(payloadConfig.collections?.map((c) => c.slug) ?? [])
    expect(slugs.has('tenants')).toBe(true)
    expect(slugs.has('posts')).toBe(true)
    expect(slugs.has('categories')).toBe(true)
    expect(slugs.has('media')).toBe(true)
  })

  it('posts collection exposes a tenant field for sandbox isolation', async () => {
    const payloadConfig = await config
    const posts = payloadConfig.collections?.find((c) => c.slug === 'posts')
    expect(fieldNames(posts?.fields)).toContain('tenant')
  })
})
...
describe.skipIf(skipDbIntegration)(
  'Multi-tenant: membership admin sandbox & full tenant stewardship',
  () => {
    let payload: Payload
