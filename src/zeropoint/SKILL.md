...
atomPath: zeropoint
coordinate: "zeropoint · 9/unity · 15ec95cc"
contentUuid: "c1b11e41-b495-5073-b3a0-a66e1c777d97"
diamondUuid: "50890d88-a6c3-8414-a203-1f86ad0bcf0f"
uuid: "15ec95cc-cd28-8c1d-9ae9-cc6307ed9b2a"
horo: 9
typography:
  partition: zeropoint
  bondDegree: 227
...
bindings: []
signatures:
  computationUuid: "72b740c4-1c1f-89f9-bdd0-a5487ad661d1"
  stages:
    - stage: path
      stageUuid: "4a319e8f-63c1-893b-85d8-f703c42a61e4"
    - stage: trinity
      stageUuid: "67b06c29-2d8a-848d-ae19-eef84ad6c73f"
    - stage: boundary
      stageUuid: "6c65e42b-2f5f-8df4-ad22-8c8743e4e197"
    - stage: links
      stageUuid: "3b117688-034d-8027-847b-720375e92b85"
    - stage: horo
      stageUuid: "ed7acfaa-f937-83fc-a0c8-c92e98948bb9"
    - stage: seal
      stageUuid: "7ccdd684-03aa-88eb-bb22-72355b3c61d7"
    - stage: uuid
      stageUuid: "15c51674-eac5-896b-a500-8efe5a0c17ad"
version: 2
---
...
- **number** — the [[rodin]] vortex / [[horo]] ring collapse irrationality into INTEGER resonance (digital-root mod-9, the {1,2,4,8,7,5,9} ring; no decimals — π is 3 in the hexagonal frame). Integers are zero-entropy.
- **identity** — the [[uuid]] singularity: every object collapses to one content-addressed 128-bit point; sprawl → a point.
- **schema** — [[collapse]]: every collection falls to the dense core (a dimension/skill/plugin), never a duplicate table.
- **convergence** — [[merge]]: same content ⇒ one; two forms of a thing are entropy until they merge.

The criterion is *selective*: [[competition]] and [[cost]] choose the minimum-entropy correct form (max order = lowest cost = the win), and creation is the only ↓entropy move (building, not destroying — see [[cost]]/[[peace]]). The source is the 0 ([[self]] — config, the akashic root) that everything radiates from and returns to ([[torus]]: implosion = collapse, explosion = supernova, one toroidal flow). erpax IS the harmonic example of zeropoint — the zero-entropy source it descends from, made an ERP society.

## No naked zero (division by zero)
erpax never divides by a bare zero. `/0` is not an error but a return to the source — the 0 everything radiates from — wearing four faces:
- **NUMBER** — 9 carries 0's residue: the [[rodin]] digital-root is mod-9 (a multiple of 9 roots to 9, the stand-in for 0), the {1,2,4,8,7,5,9} [[horo]] ring never lands on 0, and 9→1 opens a new [[dimension]] (`src/number`).
- **OPERATION** — a bare `/0` is caught at the boundary by [[utility]] (`src/utility/calculations.ts:289` `calculateRatio` returns 0 when the denominator is 0; depreciation "zero life returns 0 — no division by zero"), so no ∞ / NaN ever enters a ledger.
...
**Law — ** [[gate]] is the zeropoint enforcer: at every commit, only code that achieves the lowest-entropy form—collapsed to its dense core ([[collapse]]), merged to one canonical truth ([[merge]]), with [[proof]] of integrity—passes through. [[refactor]] toward zero entropy is not optional elegance; it is the tamper-resistant Merkle that makes the codebase append-only and verification-inevitable ([[integrity]]). The commit is the moment zeropoint is measured.
