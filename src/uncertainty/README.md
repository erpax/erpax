<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# uncertainty

> atom `uncertainty` · horo `7` `descent` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[trinity]]/code `1` |  |
| [[asset]]/[[trinity]]/proof `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `12` |  |
| Σ `21` | Σ `0` |

> [[balance]] `21` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `uncertainty` · sub-root `e584c996-ed05-8239-ace8-54f7bef8dc80`
- bond degree `21` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `e584c996-ed05-8239-ace8-54f7bef8dc80`
- bonds in `6` · out `6`
- trinity form·code·proof `1`·`1`·`1`
- links `16` / `16`
- folded `1` · escapes `0`

## typography graph

- partition `uncertainty`
- partition root `e584c996-ed05-8239-ace8-54f7bef8dc80`
- bond degree `21`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `7255775e-b70f-8478-939b-322db58a1245` · framed by typography partition `uncertainty` bonds `21` · `pnpm readme` · `pnpm readme:check`</sub>
