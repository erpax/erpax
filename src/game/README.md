<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# game

> atom `game` · horo `2` `share` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `114` |  |
| Σ `121` | Σ `0` |

> [[balance]] `121` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `game` · sub-root `fdfd5a19-a098-8016-8d6c-8dd6babad4b2`
- bond degree `144` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `fdfd5a19-a098-8016-8d6c-8dd6babad4b2`
- bonds in `58` · out `56`
- trinity form·code·proof `1`·`0`·`0`
- links `17` / `17`
- folded `1` · escapes `0`

## typography graph

- partition `game`
- partition root `fdfd5a19-a098-8016-8d6c-8dd6babad4b2`
- bond degree `144`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `c4fb6e8a-a57c-8544-bdad-7773c840c5e6` · framed by typography partition `game` bonds `144` · `pnpm readme` · `pnpm readme:check`</sub>
