...
interface InvoiceDoc {
  ...
  buyerAddress?: import('@/saf/t').SafTAddressStructure
  amounts?: {
    netTotal?: number
    taxTotal?: number
    totalAmount?: number
...
export const buildSalesInvoices = async (
  ...
  startDate: string,
  endDate: string,
): Promise<NonNullable<import('@/saf/t').SafTSourceDocuments['salesInvoices']>> => {
  const result = await payload.find({
    collection: 'invoices',
    where: {
      and: [
...
export const buildPurchaseInvoices = async (
  ...
  startDate: string,
  endDate: string,
): Promise<NonNullable<import('@/saf/t').SafTSourceDocuments['purchaseInvoices']>> => {
  const result = await payload.find({
    collection: 'invoices',
    where: {
      and: [
...
const PAYMENT_MECH_MAP: Record<string, import('@/saf/t').SafTPaymentMechanism> = {
  cash: 'NU',
  cheque: 'CH',
  bank_transfer: 'TB',
  wire: 'TB',
...
export const buildPayments = async (
  ...
  startDate: string,
  endDate: string,
): Promise<NonNullable<import('@/saf/t').SafTSourceDocuments['payments']>> => {
  const result = await payload.find({
    collection: 'payments',
    where: {
      and: [
...
): import('@/saf/t').SafTMovementOfGoods => ({
...
export const buildMovementOfGoods = async (
  ...
  startDate: string,
  endDate: string,
): Promise<NonNullable<import('@/saf/t').SafTSourceDocuments['movementOfGoods']>> => {
  const result = await payload.find({
...
export const buildSourceDocuments = async (
  ...
  startDate: string,
  endDate: string,
): Promise<import('@/saf/t').SafTSourceDocuments | undefined> => {
...
const renderSalesInvoiceLine = (
  l: import('@/saf/t').SafTSalesInvoiceLine,
): string =>
  wrap(
    'Line',
...
const renderSalesInvoice = (
  inv: import('@/saf/t').SafTSalesInvoice,
): string =>
  wrap(
    'Invoice',
...
const renderMovementOfGoods = (
  m: import('@/saf/t').SafTMovementOfGoods,
): string =>
  wrap(
    'StockMovement',
...
const renderSourceDocuments = (
  src: import('@/saf/t').SafTSourceDocuments,
): string => {
