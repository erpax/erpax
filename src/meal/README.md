<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# meal

> atom `meal` · horo `1` `base` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `55` |  |
| Σ `62` | Σ `0` |

> [[balance]] `62` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `meal` · sub-root `fdfe3e77-697e-84a5-9b0e-c78834fdc1ec`
- bond degree `61` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `fdfe3e77-697e-84a5-9b0e-c78834fdc1ec`
- bonds in `27` · out `28`
- trinity form·code·proof `1`·`0`·`0`
- links `5` / `5`
- folded `1` · escapes `0`

## typography graph

- partition `meal`
- partition root `fdfe3e77-697e-84a5-9b0e-c78834fdc1ec`
- bond degree `61`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `300c8c54-be21-839e-9c42-9b47def26f5c` · framed by typography partition `meal` bonds `61` · `pnpm readme` · `pnpm readme:check`</sub>
