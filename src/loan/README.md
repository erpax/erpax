<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# loan

> atom `loan` · horo `4` `weave` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `8` |  |
| Σ `15` | Σ `0` |

> [[balance]] `15` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `loan` · sub-root `43f735df-869a-8b0b-91a9-1e366e0f3dc4`
- bond degree `35` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `43f735df-869a-8b0b-91a9-1e366e0f3dc4`
- bonds in `4` · out `4`
- trinity form·code·proof `1`·`0`·`0`
- links `18` / `18`
- folded `1` · escapes `0`

## typography graph

- partition `loan`
- partition root `43f735df-869a-8b0b-91a9-1e366e0f3dc4`
- bond degree `35`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `96499fb2-89f0-8852-90c6-29896b06fe19` · framed by typography partition `loan` bonds `35` · `pnpm readme` · `pnpm readme:check`</sub>
