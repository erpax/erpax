<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# notice

> atom `notice` · horo `8` `crest` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `154` |  |
| Σ `161` | Σ `0` |

> [[balance]] `161` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `notice` · sub-root `2b7f414d-e0e6-81ce-bb32-4e0407263e18`
- bond degree `160` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `2b7f414d-e0e6-81ce-bb32-4e0407263e18`
- bonds in `77` · out `77`
- trinity form·code·proof `1`·`0`·`0`
- links `5` / `5`
- folded `1` · escapes `0`

## typography graph

- partition `notice`
- partition root `2b7f414d-e0e6-81ce-bb32-4e0407263e18`
- bond degree `160`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `dbfb88ac-505b-898e-8aa7-0a3ca080078b` · framed by typography partition `notice` bonds `160` · `pnpm readme` · `pnpm readme:check`</sub>
