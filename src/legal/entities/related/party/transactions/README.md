<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# transactions

> atom `legal/entities/related/party/transactions` · horo `8` `crest` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[trinity]]/code `1` |  |
| [[asset]]/[[trinity]]/proof `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `51` |  |
| Σ `60` | Σ `0` |

> [[balance]] `60` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `legal` · sub-root `1fd2f1ff-ccee-87e0-8d80-12345443fed8`
- bond degree `83` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `de64d7c1-15fd-8ae5-a415-173b51516c8d`
- bonds in `22` · out `29`
- trinity form·code·proof `1`·`1`·`1`
- links `1` / `1`
- folded `1` · escapes `0`

## typography graph

- partition `legal`
- partition root `1fd2f1ff-ccee-87e0-8d80-12345443fed8`
- bond degree `83`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `93c360a8-a613-8efe-b945-69112ebd9067` · framed by typography partition `legal` bonds `83` · `pnpm readme` · `pnpm readme:check`</sub>
