<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# command

> atom `payload/command` · horo `4` `weave` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[trinity]]/code `1` |  |
| [[asset]]/[[trinity]]/proof `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `40` |  |
| Σ `49` | Σ `0` |

> [[balance]] `49` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `payload` · sub-root `fff1cd42-9f30-8151-8e61-e5d925926388`
- bond degree `67` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `5b279972-c7eb-8269-b0fa-f56ea10676c0`
- bonds in `20` · out `20`
- trinity form·code·proof `1`·`1`·`1`
- links `11` / `11`
- folded `1` · escapes `0`

## typography graph

- partition `payload`
- partition root `fff1cd42-9f30-8151-8e61-e5d925926388`
- bond degree `67`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `0343aaf9-2dae-81d8-97db-79ff0ebce057` · framed by typography partition `payload` bonds `67` · `pnpm readme` · `pnpm readme:check`</sub>
