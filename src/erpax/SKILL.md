...
It also states, in code, the two things a consumer is actually bound by: the one law, and the tiered licence (`src/algebra/**` is MIT; the corpus is CC-BY-NC-ND-4.0, commercial terms via `license@erpax.com`). Stating them where the consumer meets them beats a LICENSE file they may never open.
