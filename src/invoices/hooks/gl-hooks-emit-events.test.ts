  ...
  it('payment hook emits payment:received for incoming AR collection', async () => {
    await runHook(paymentAccountingHook, {
      doc: {
        ...
        partyId: 'cust-1',
        // canonical schema: amount/currency/date/method live in field groups (payment.ts reads
        // amounts.amount · dates.receivedAt · payment.paymentMethod). paymentKind stays top-level.
        amounts: { amount: 1_200, currencyCode: 'EUR' },
        dates: { receivedAt: '2026-05-09' },
  ...
  it('payment hook emits payment:sent for outgoing AP disbursement', async () => {
    await runHook(paymentAccountingHook, {
      doc: {
        ...
        paymentKind: 'sent',
        partyId: 'vendor-1',
        amounts: { amount: 500, currencyCode: 'EUR' },
        dates: { sentAt: '2026-05-09' },
