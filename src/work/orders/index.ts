 ...
 * in the lot funnel (`@/lots` → `@/lot/variants` → `@/lot/work/phases`).
 *
 ...
 * @see src/lots/index.ts · src/lot/work/phases/index.ts (the lot funnel = the planning/header layer)
...
// Harmony gate — a disharmonious ring is a build-time failure (the horo law).
{
  ...
  if (JSON.stringify(steps) !== JSON.stringify([...HORO_DIGITS])) {
    throw new Error(`work-orders: horo disharmony — expected ${HORO_DIGITS.join(',')}, got ${steps.join(',')}`)
  }
  for (const s of WORKORDER_RING) {
    if (!isHoroStep(s.step)) throw new Error(`work-orders: state ${s.code} step ${s.step} is off-ring (escape)`)
  }
}
