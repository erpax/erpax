/**
 * Tenant-domain deep-slug page renderer.
 *
...
export default async function Page({
  ...
  try {
    ...
    if (tenantsQuery.docs.length === 0) {
      redirect(
        `/tenant-domains/${encodeURIComponent(params.tenant)}/login?redirect=${encodeURIComponent(
          `/tenant-domains/${params.tenant}${slug?.length ? `/${slug.join('/')}` : ''}`,
        )}`,
      )
    }
  } catch {
