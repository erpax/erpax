...
/** Credit for claims in src/app/(frontend)/[locale]/products/page.tsx — chatHealLeftoverWave; not an empty gaming test. */
describe('src/app/(frontend)/[locale]/products/page.tsx — leftover wave proof', () => {
  it('source still exports its claimed surface and claim markers (refutable — deleting them fails)', async () => {
    ...
    expect(src).toMatch(/\bexport\s+const\s+metadata\b/)
