...
The live tree carries a known backlog of pre-law folders; driving it to zero is a tree-wide rename + trinity-authoring migration (separate scope). So, exactly like [[convention]]/import, the gate is a **ratchet**: `folderRatchet({ violations, baseline })` fails when the live count exceeds the committed `FOLDER_LAW_BASELINE`. The folder law therefore **cannot get worse** — every new malformed folder is a red gate in the live [[confirm]] hook, in pre-push, and in CI [[test]] — so no agent can introduce one in any session. The baseline moves only DOWN, in the same diff that removes folders. Zero ⇒ tamper-[[cost]] → ∞.
