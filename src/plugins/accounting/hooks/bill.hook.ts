/**
 * Bill GL Posting Hook — auto-creates GL entry on bill change.
 *
 * Debit expense account (per category), Credit 2100 (AP).
 *
  ...
  try {
    ...
    if (glPostingService) {
      const billData = {
        ...
        items: doc.lineItems || [],
        description: `Bill ${doc.billNumber}`,
      };

      await glPostingService.postBill(tenant, billData);
      req.payload.logger.info(`✓ GL posting created for bill ${doc.billNumber}`);
    }
  } catch (error) {
    req.payload.logger.error({ err: error }, `✗ Error creating GL posting for bill:`);
  }
