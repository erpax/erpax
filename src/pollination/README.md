<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# pollination

> atom `pollination` · horo `4` `weave` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `30` |  |
| Σ `37` | Σ `0` |

> [[balance]] `37` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `pollination` · sub-root `378f1d8a-fb1c-8f77-bd9c-cddb44faa9d2`
- bond degree `43` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `378f1d8a-fb1c-8f77-bd9c-cddb44faa9d2`
- bonds in `15` · out `15`
- trinity form·code·proof `1`·`0`·`0`
- links `21` / `21`
- folded `1` · escapes `0`

## typography graph

- partition `pollination`
- partition root `378f1d8a-fb1c-8f77-bd9c-cddb44faa9d2`
- bond degree `43`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `9fd600eb-120b-82c5-8305-b49df080b704` · framed by typography partition `pollination` bonds `43` · `pnpm readme` · `pnpm readme:check`</sub>
