...
describe('providers barrel — the shipped providers register their roles', () => {
  it('registering each shipped provider directly populates the expected role set', async () => {
    ...
    const { InternalSigningProvider } = await import('@/self/closure/provider/signing')
    const { InternalFederationProvider } = await import('@/self/closure/provider/federation')
    const { InternalNotificationProvider } = await import('@/self/closure/provider/notification')
    const { InternalSearchProvider } = await import('@/self/closure/provider/search')
    for (const p of [
