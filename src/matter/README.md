<!-- GENERATED by src/readme/index.ts — debit/credit statement; do NOT edit by hand. -->

# matter

> atom `matter` · horo `5` `round` · [[balance]] `0` · [[seal]] `0`

## [[debit]] · [[credit]]

| [[debit]] | [[credit]] |
| -------- | --------- |
| [[asset]]/[[trinity]]/form `1` |  |
| [[asset]]/[[lattice]]/folded `1` |  |
| [[asset]]/[[links]]/resolved `1` |  |
| [[asset]]/[[horo]]/ring `1` |  |
| [[asset]]/[[identity]]/uuid `1` |  |
| [[asset]]/[[boundary]]/barrel `1` |  |
| [[asset]]/[[typography]]/partition `1` |  |
| [[asset]]/[[bonds]]/degree `10` |  |
| Σ `17` | Σ `0` |

> [[balance]] `17` · [[conservation]] `0` · [[entry]] · [[purity]]

## typography graph

- partition `matter` · sub-root `9fa22fa3-5612-889f-8501-aa0f526dd30a`
- bond degree `58` · analysis neighbors —
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## identity

- uuid `9fa22fa3-5612-889f-8501-aa0f526dd30a`
- bonds in `5` · out `5`
- trinity form·code·proof `1`·`0`·`0`
- links `35` / `35`
- folded `1` · escapes `0`

## typography graph

- partition `matter`
- partition root `9fa22fa3-5612-889f-8501-aa0f526dd30a`
- bond degree `58`
- analysis neighbors `0`
- graph root `8e0333cd-772f-8747-b779-904b57e10b0d`

## [[seal]]

- `unsealed` — [[purity]] · [[seal]] · [[diamond]]

---

<sub>content-uuid `694b77e7-11af-8fcc-bb93-fa9ae5c146a1` · framed by typography partition `matter` bonds `58` · `pnpm readme` · `pnpm readme:check`</sub>
